...
static void spi_txe_interrupt_handle(SPI_Handle_t *pSPIHandle){
				...
				if(! pSPIHandle ->TxLen){
					//TxLen is Zero, so create the spi Transmission and inform the application that
					//TX is over.
					// this prevents interrupts from setting up of TXE flag
					SPI_CloseTransmisson(pSPIHandle);

					// Now we will tell the application
...
static void spi_ovr_err_interrupt_handle(SPI_Handle_t *pSPIHandle){
	...
	uint8_t temp ;
	//Clear the OVR flag and then inform the application
...
void SPI_CloseTransmisson(SPI_Handle_t *pSPIHandle){
		...
		pSPIHandle->pSPIx->CR2 &= ~(1 << SPI_CR2_TXEIE);
		pSPIHandle->pTxBuffer = NULL;  // now we make NULL
		pSPIHandle->TxLen = 0;
...
void SPI_CloseReception(SPI_Handle_t *pSPIHandle){
		...
		pSPIHandle->pSPIx->CR2 &= ~(1 << SPI_CR2_RXNEIE);
		pSPIHandle->pRxBuffer = NULL;  // now we make NULL
		pSPIHandle->RxLen = 0;
