...
/*
//Section 66, L236 check for replies in questions section
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t AppEv)
{
	static uint32_t cnt = 0;
	static uint32_t w_ptr = 0;

	uint32_t data_len = strlen(Tx_buf[32]);
	...
	}else if (AppEv == I2C_EV_DATA_REQ)
	{
		...
		if(CommandCode == 0x51)
		{
			...
			I2C_SlaveSendDataIT(I2C1,((data_len >> ((cnt%4) * 8)) & 0xFF));
		    cnt++;
		}else if (CommandCode == 0x52)
		{
			//sending Tx_buf contents indexed by w_ptr variable
			I2C_SlaveSendDataIT(I2C1, Tx_buf[w_ptr++]);
		}
	}else if (AppEv == I2C_EV_DATA_RCV)
	{
		//Master has sent command code, read it
		 CommandCode = I2C_SlaveReceiveDataIT(I2C1);
...
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle,uint8_t AppEv)
{
	...
	commandCode = pI2CHandle->pI2Cx->DR;
	...
	if(AppEv == I2C_EV_DATA_REQ)
	{
		...
		if(commandCode == 0x51)
		{
			...
			I2C_SlaveSendDataIT(pI2CHandle->pI2Cx,strlen((char*)Tx_buf));
		}else if (commandCode == 0x52)
		{
			//Send the contents of Tx_buf
			I2C_SlaveSendDataIT(pI2CHandle->pI2Cx,Tx_buf[Cnt++]);
