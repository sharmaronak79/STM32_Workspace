...
int main(void)
{
  ...
  HAL_SPI_Transmit(&hspi1, (uint8_t*)&LMK_WREN	,1, 100);
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_RESET);

  //Read Status Register
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, (uint8_t*)&LMK_RDSR	,1, 100);
  HAL_SPI_Receive(&hspi1, (uint8_t*)spi_buf,1, 100);
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_SET);

  //Printout status register
  sprintf(uart_buf, "Status : 0x%02x \r\n",(unsigned int)spi_buf[0]);
  HAL_UART_Transmit(&huart3, (uint8_t*)uart_buf, strlen(uart_buf), 100);



  //Test bytes to write to LMX
  //E.g
  spi_buf[0]=0xAB;
  spi_buf[1]=0xCD;
  spi_buf[2]=0xEF;
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, (uint8_t*)&LMK_WRITE, 1, 100);
  ...
  */
  ...
  //Read data
  //1.Put CS low to Activate(to make connection) the SPI slave device
  HAL_GPIO_WritePin(GPIOF, GPIO_PIN_12, GPIO_PIN_RESET);
  //2. Transmit Register Address
 spiData[0]=0x05; // SPI Register address,
  HAL_SPI_Transmit(&hspi1, spiData, 1, 100); // this will make connection with this addressed Register of SPI slave device


  //3.Read
  HAL_SPI_Receive(&hspi1, spiData, 5, 100);
  //4. bring CS pin high to break the connection with Slave device
  HAL_GPIO_WritePin(GPIOF, GPIO_PIN_12, GPIO_PIN_SET);

  sprintf((char*)Buf,"\nReceived Data is : 0x%X  0x%X  0x%X  0x%X  0x%X",spiData[0],spiData[1],spiData[2],spiData[3],spiData[4]);
  HAL_UART_Transmit(&huart3, Buf, strlen((char*)Buf), HAL_MAX_DELAY);

  sprintf((char*)Buf,"\nReceived Data is : %s",spiData);
   HAL_UART_Transmit(&huart3, Buf, strlen((char*)Buf), HAL_MAX_DELAY);
